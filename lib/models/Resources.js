...
export default class Resources {
  ...
  static async insert({ title, category_id, about, website, logo }) {
    const { rows } = await pool.query(
      'INSERT INTO resources (title, category_id, about, website, logo) VALUES ($1, $2, $3, $4, $5) RETURNING *',
      [
        title,
        category_id,
        about,
  ...
  static async update(
    id,
    { title, category_id, about, website, logo }
  ) {
    const existingResource = await Resources.getById(id);
    const new_title = title ?? existingResource.title;
    const new_category_id = category_id ?? existingResource.category_id;
    ...
    const { rows } = await pool.query(
      'UPDATE resources SET title=$1, category_id=$2, about=$3, website=$4, logo=$5 WHERE id=$6 RETURNING *',
      [
        new_title,
        new_category_id,
        new_about,
